...
import (
	...
	apisCluster "github.com/terasky-oss/provider-harness/apis/cluster"
	apisNamespaced "github.com/terasky-oss/provider-harness/apis/namespaced"
	"github.com/terasky-oss/provider-harness/config"
	"github.com/terasky-oss/provider-harness/internal/clients"
	controllerCluster "github.com/terasky-oss/provider-harness/internal/controller/cluster"
	controllerNamespaced "github.com/terasky-oss/provider-harness/internal/controller/namespaced"
	"github.com/terasky-oss/provider-harness/internal/features"
	"github.com/terasky-oss/provider-harness/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Harness").DefaultEnvars()
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped Harness APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced Harness APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped Harness controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Harness controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped Harness controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced Harness controllers")
	}
